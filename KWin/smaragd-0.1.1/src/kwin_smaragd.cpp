...
static QRegion getBlurMask(QSize s, int radius)
{
	QPainterPath path;
...
void Decoration::updateLayout()
{
    ...
    setBorders(QMargins(
        ...
        !maximized ? ws->top_space + ws->normal_top_corner_space + ws->titlebar_height : ws->titlebar_height + ws->top_space - 2,
	...
	setBlurRegion(getBlurMask(size(), maximized ? 0 : 6));
...
void Decoration::paint(QPainter *painter, const QRect &repaintArea)
{
    ...
    QRect captionRect(m_buttonGroup[0]->geometry().right() + 4 + (client().data()->isMaximized() ? 5 : 0), 0, m_buttonGroup[2]->geometry().left() - m_buttonGroup[0]->geometry().right() - 4, borderTop());
